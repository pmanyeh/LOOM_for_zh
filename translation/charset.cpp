...
byte *ScummEngine::get2byteCharPtr(int idx) {
	if (!isScummvmKorTarget() && (_game.platform == Common::kPlatformFMTowns || _game.platform == Common::kPlatformPCEngine)) {
		if (_language != Common::ZH_CHN && _language != Common::ZH_TWN)
			return nullptr;
	}
...
void CharsetRendererV3::printChar(int chr, bool ignoreCharsetMask) {
	...
	// ----- CJK FIX FOR V3 -----
	static int pendingCJKByte = 0;
	if (_vm->_useCJKMode && (_vm->_language == Common::ZH_CHN || _vm->_language == Common::ZH_TWN)) {
		if (chr >= 256) {
			// Already combined by drawString! Proceed normally.
			pendingCJKByte = 0;
		} else if (pendingCJKByte) {
			// Second byte! Combine them.
			chr = pendingCJKByte | (chr << 8);
			pendingCJKByte = 0;
		} else if (chr & 0x80) {
			// First byte! Buffer it and wait.
			pendingCJKByte = chr;
			return; 
		} else {
			// Normal ASCII byte.
...
CharsetRendererTownsV3::CharsetRendererTownsV3(ScummEngine *vm) : CharsetRendererV3(vm), _sjisCurChar(0) {
	if (!_vm->_useCJKMode && _vm->_game.id == GID_LOOM && _vm->_game.platform == Common::kPlatformFMTowns) {
		_vm->_language = Common::ZH_CHN;
		_vm->loadCJKFont();
	}
}
...
#ifdef USE_RGB_COLOR
	if (_vm->_language == Common::JA_JPN && _vm->_useCJKMode && chr > 127) {
		assert(_vm->_cjkFont);
...
void CharsetRendererTownsV3::setDrawCharIntern(uint16 chr) {
	_sjisCurChar = (!_vm->isScummvmKorTarget() && _vm->_language == Common::JA_JPN && _vm->_useCJKMode && chr > 127) ? chr : 0;
}
#endif
